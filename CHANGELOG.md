...
- Updated the shipped `.trebired/logger/config.ts` `forVersion` to `2.7.0` and the `@trebired/utils` dependency range to `^0.9.3`. Ranges permitting 0.9.0 through 0.9.2 must be avoided: those releases throw when a caller validates a version without passing the config object.
