...
const PACKAGE_CONFIG_ORGANIZATION_NAME = cleanSegment(
  packageJson.config?.organization?.name,
);
const PACKAGE_NAME =
PACKAGE_JSON_NAME ||
  (PACKAGE_CONFIG_ORGANIZATION_NAME
  ? `@${PACKAGE_CONFIG_ORGANIZATION_NAME}/startup`
  : "startup");
const PACKAGE_VERSION = PACKAGE_JSON_VERSION || "0.1.0";
const PACKAGE_ORGANIZATION_NAME =
PACKAGE_CONFIG_ORGANIZATION_NAME || packageScope(PACKAGE_JSON_NAME);
const PACKAGE_SLUG = packageSlug(PACKAGE_NAME) || "startup";
