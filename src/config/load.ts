...
async function loadConfig(
  ...
  if (!await pathExists(configPath)) throw new Error(`startup config was not found: ${configPath}`);
...
function loadConfigSync(
  ...
  if (!configPath) return handleMissingConfig(options);
  if (!fs.existsSync(configPath)) throw new Error(`startup config was not found: ${configPath}`);
...
async function findConfig(startDir = process.cwd(), boundaryDir?: string): Promise<string|null> {
  ...
  for (;; ) {
    const candidate = path.join(current, STARTUP_PROJECT_CONFIG_PATH);
...
function runConfigSource(source: string, configPath: string): unknown {
  try {
    return Function(
      "defineConfig",
      `${toRuntimeConfigSource(source, configPath)}\n//# sourceURL=${pathToFileURL(configPath).href}`,
    )(defineConfig);
  } catch (error) {
    const reason = error instanceof Error ? error.message : String(error);
    throw new Error(`startup config failed to load: ${configPath}: ${reason}`);
  }
}
...
function toRuntimeConfigSource(source: string, configPath: string): string {
  ...
  if (/\bexport\b/u.test(runtimeSource)) {
    throw new Error(`startup config only supports a default export: ${configPath}`);
  }
